...
def open_display_window(width, height):
    ...
    cv2.setWindowTitle(WINDOW_NAME, 'Detector')
...
def loop_and_detect(cam, tf_sess, conf_th, vis, od_type):
    ...
    distance = 0
    mid_x,mid_y = 0,0
    global ser,height,width  #Vincent
    while True:
        ...
        if img is not None:
            ...
            region_of_interest_vertices = [(0,(height-20)),(width/2,height/2),(width,(height-20))]
            ...
            for cl in cls:
                cl = int(cl)
                if(cl == 1):
                    print("[INFO] Arm Detected")
                    color = 10
                elif(cl == 2):
                    print("[INFO] Box Detected => Action")
                    color = 20
                elif(cl == 3):
                    print("[INFO] Human Detected => Stop")
                    cv2.putText(img, "Danger", (100, 100), font, 1.0, (240, 240, 240), 1, line)
                    color = 30
                #elif(cl == 4):
                    #print("[INFO] GREEN(RIGHT) Traffic Light Detected => RIGHT")
                    #color = 40
            #Draw boundary boxes
            img = vis.draw_bboxes(img, box, conf, cls)
            #Draw camera reference
            cv2.circle(img,(320,340),4,(0,255,0),-1)
            cv2.putText(img,"REFERENCE",(320-10,340-10),cv2.FONT_HERSHEY_SIMPLEX,0.5,(0,255,0),2)
            ...
            cv2.imshow("Canny-Detector",edges)
            toc = time.time()
